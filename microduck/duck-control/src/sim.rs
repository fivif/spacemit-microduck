...
impl RobotIo for RemoteIo {
    ...
    /// A no-op: the sim protocol has no reboot op, and a simulated servo has no firmware to
    /// restart. Accepted rather than refused so recovery paths that reboot a wedged servo do not
    /// turn into a second failure in simulation.
    fn reboot(&mut self, _id: u8) -> Result<()> {
        Ok(())
    }
