...
struct Args {
    ...
    /// Run against a body in MuJoCo, served over TCP by `microduck_rl`'s `duck-body`.
    ///
    /// **The same daemon, a simulated body.** Everything above `duck_control::io::RobotIo` — the
    /// 50 Hz loop, the policies, safety, fall detection, odometry, every IPC call — is the code
    /// that runs on a robot, unchanged and unable to tell. `duck_control::sim` has the protocol
    /// and the reasons for its shape.
    ///
...
fn spawn_control_thread(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // A simulated body. No `open_bus_waiting` equivalent, deliberately: `RemoteIo`
            // connects lazily and reconnects on every tick, so a duck started before its
            // simulator simply reports unhealthy until the simulator answers — which is what a
            // robot with no power on the bus does too, by a different route.
