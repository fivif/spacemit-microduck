...
> **状态**: 🟡 P0' 进行中(2026-09-08 板卡确认 + 环境就绪,裁剪构建未跑完)
> **目标**: 在 **SpacemiT K1(MUSE-Pi-Pro)** 上复刻 K3 的 P0/P1 —— 原生编译的 `robotd` 驱动 Microduck。
...
| **ORT** | ✅ 预装 **1.24.2+spacemit.a1** | ❌ apt 只有 **1.18.1** | ⚠️ **R1 爆发点**,见 `docs/04` |
...
- [ ] 板子回来后在 `/tmp/k1-build.log` 查构建结果(或重跑 `build-k1.sh`)
- [ ] 解决 R1:让 `ort rc.11` 吃上可用的 `libonnxruntime.so`(≥1.23)
- [ ] 复刻 P1:`robotd --sim` 接本地仿真跑通起身→站立→行走
- [ ] 与 K3 做算力/环率对比(同策略同场景)
